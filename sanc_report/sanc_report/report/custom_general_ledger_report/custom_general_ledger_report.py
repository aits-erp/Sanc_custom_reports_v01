...
def get_gl_entries(filters, accounting_dimensions):
	...
	# Build party name map once for all party types
	party_name_map = get_party_name_map()

	for gl_entry in gl_entries:
		# Resolve party_name for the Party column
		if gl_entry.party_type and gl_entry.party:
			gl_entry.party_name = party_name_map.get(gl_entry.party_type, {}).get(
				gl_entry.party, gl_entry.party
			)

		# Resolve against field: replace party codes with display names
		if gl_entry.against:
			gl_entry.against = resolve_against_display_name(
				gl_entry.against, gl_entry.voucher_type, party_name_map
			)
...
# ---------------------------------------------------------------------------
# NEW HELPER: resolves party codes in the "against" field to display names
# ---------------------------------------------------------------------------
def resolve_against_display_name(against, voucher_type, party_name_map):
	"""
	The GL Entry 'against' field stores a comma-separated list of
	account names or party codes.  This function replaces any token
	that matches a known party code with the human-readable display name.

	Voucher-type mapping used for lookup priority:
	  Sales Invoice / Sales Order / Delivery Note  → Customer
	  Purchase Invoice / Purchase Order / Purchase Receipt → Supplier
	  Payment Entry  → both Customer and Supplier (check both)
	  Journal Entry  → check all party types
	  Expense Claim / Payroll Entry                → Employee
	  All others     → check all party types as fallback
	"""

	if not against:
		return against

	# Decide which party types to check based on voucher_type
	voucher_to_party_types = {
		...
		"Payment Entry":       ["Customer", "Supplier", "Employee"],
		"Journal Entry":       ["Customer", "Supplier", "Employee"],
	}

	party_types_to_check = voucher_to_party_types.get(
		voucher_type,
		["Customer", "Supplier", "Employee"]   # fallback: check all three
	)

	# Build a flat lookup: code → display name (only for relevant party types)
	flat_map = {}
	for pt in party_types_to_check:
		for code, display_name in party_name_map.get(pt, {}).items():
			if display_name and display_name != code:
				flat_map[code] = display_name

	# Replace each comma-separated token if it exists in the flat map
	tokens = [t.strip() for t in against.split(",") if t.strip()]
	resolved = [flat_map.get(token, token) for token in tokens]
...
def get_party_name_map():
	"""
	Returns a nested dict of party code → display name for all supported party types:
	  {
	    "Customer":  { "CUST-0001": "Acme Corp", ... },
	    "Supplier":  { "SUPP-0001": "Global Supplies Ltd", ... },
	    "Employee":  { "EMP-0001":  "John Doe", ... },
	  }
	"""
	party_map = {}
...
def get_columns(filters):
	...
	columns += [
		...
		# Against Account column now shows resolved party display names
		{"label": _("Against Account"), "fieldname": "against", "width": 200},

...
from erpnext.accounts.report.sanc_report.custom_general_ledger_report.custom_general_ledger_report import execute
from erpnext.controllers.sales_and_purchase_return import make_return_doc


class TestCustomGeneralLedgerReport(FrappeTestCase):
	def setUp(self):
	...
	def test_foreign_account_balance_after_exchange_rate_revaluation(self):
		...
		# check if custom general ledger report shows correct balance
	...
	def test_ignore_cr_dr_notes_filter(self):
		...
		# With ignore_cr_dr_notes enabled
	...
	def test_against_account_shows_customer_name(self):
		"""
		Verify that the Against Account field shows customer_name
		instead of the raw customer code for Sales Invoice entries.
		"""
		si = create_sales_invoice()
		...
		customer_name = frappe.db.get_value("Customer", si.customer, "customer_name")
		gl_rows = [d for d in data if d.get("voucher_no") == si.name]

		self.assertTrue(len(gl_rows) > 0, "No GL rows found for the Sales Invoice")

		for row in gl_rows:
			if row.get("against"):
				# Against field should contain the customer_name, not just the raw code
				self.assertIn(
					customer_name,
					row.against,
					f"Expected customer_name '{customer_name}' in against field, got: '{row.against}'"
				)

	def test_against_account_shows_supplier_name(self):
		"""
		Verify that the Against Account field shows supplier_name
		instead of the raw supplier code for Purchase Invoice entries.
		"""
		...
		supplier_name = frappe.db.get_value("Supplier", pi.supplier, "supplier_name")
		gl_rows = [d for d in data if d.get("voucher_no") == pi.name]

		self.assertTrue(len(gl_rows) > 0, "No GL rows found for the Purchase Invoice")
		...
		for row in gl_rows:
			if row.get("against"):
				self.assertIn(
					supplier_name,
					row.against,
					f"Expected supplier_name '{supplier_name}' in against field, got: '{row.against}'"
				)
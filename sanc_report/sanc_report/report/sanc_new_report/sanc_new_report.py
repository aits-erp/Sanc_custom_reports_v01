...
#             AND po.docstatus = 1
#             AND po.status NOT IN ('Cancelled', 'Closed', 'On Hold')
...
#             AND po.docstatus = 1
#             AND po.status NOT IN ('Cancelled', 'Closed', 'On Hold')
#             AND poi.custom_good_in_transit = 1
#         """, (code,))[0][0] or 0
...
def get_data(filters):
    ...
    # Month boundaries (same as standard report logic)
    m1_start = (base_date - relativedelta(months=1)).replace(day=1)
    m1_end   = base_date.replace(day=1)

    m2_start = (base_date - relativedelta(months=2)).replace(day=1)
    m2_end   = m1_start

    m3_start = (base_date - relativedelta(months=3)).replace(day=1)
    m3_end   = m2_start
    ...
    for item in items:
        ...
        # ---- UPDATED: Use Sales Invoice stock_qty (actual dispatched qty) ----
        m1 = get_sales_invoice_qty(code, m1_start, m1_end)
        m2 = get_sales_invoice_qty(code, m2_start, m2_end)
        m3 = get_sales_invoice_qty(code, m3_start, m3_end)

        # ---- SANC STOCK: Use tabBin for real-time current stock ----
        stock = frappe.db.sql("""
            SELECT COALESCE(SUM(actual_qty), 0)
            FROM `tabBin`
            WHERE item_code = %s
...
# ---------------- SALES (Sales Invoice stock_qty - same as standard report) ----------------
def get_sales_invoice_qty(item_code, start, end):
    """
    Returns total stock_qty from submitted Sales Invoices for the given item
    within [start, end) date range — mirrors the standard Item-Wise Sales
    History report's stock_qty column logic.
    """

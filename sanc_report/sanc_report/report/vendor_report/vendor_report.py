...
def get_columns():
	"""
	Same column structure as the existing Vendor Report.

	Transaction Type:
	- Source: Payment Entry.custom_transaction_type
	- Data field
	- Editable directly in the report
	- Edited value is saved back to the same Payment Entry

	The 4 Blank spacer columns remain unchanged.
	"""
	...
	return [
		...
		# It identifies the exact Payment Entry to update.
		{
			"label": _("Payment Entry"),
			"fieldname": "_payment_entry_name",
			"fieldtype": "Data",
			"hidden": 1,
		},
	]
...
def get_data(filters):
	...
	for row in raw_rows:
		...
		bene_address_1 = row.get(
			"bene_address_1"
		)

		bene_address_2 = row.get(
			"bene_address_2"
		)

		bene_address_3 = row.get(
			"bene_address_3"
		)

		bene_address_4 = row.get(
			"bene_address_4"
		)

		bene_address_5 = row.get(
			"bene_address_5"
		)
		...
		payment_details_1 = row.get(
			"payment_details_1"
		)

		payment_details_2 = row.get(
			"payment_details_2"
		)

		payment_details_3 = row.get(
			"payment_details_3"
		)

		payment_details_4 = row.get(
			"payment_details_4"
		)

		payment_details_5 = row.get(
			"payment_details_5"
		)

		payment_details_6 = row.get(
			"payment_details_6"
		)

		payment_details_7 = row.get(
			"payment_details_7"
		)
		...
		# RBI NOTEPAD DATA
		#
		# Transaction Type is the actual value from
		# Payment Entry.custom_transaction_type.
		#
		# The JS replaces the first field with the current
		# edited value before downloading.
		# ------------------------------------------------------------
...
def get_raw_rows(filters):
	"""
	Report is anchored on Payment Entry.

	Only submitted Supplier Payment Entries are included.

	Transaction Type is taken DIRECTLY from:
	Payment Entry.custom_transaction_type

	No conversion to I/N/R/M is performed.
	"""
...
def get_supplier_email(party):
	"""
	Fetch Contact.email_id for Supplier.
	"""
...
def get_party_address(
	...
	"""
	Fetch Address linked to Payment Entry party.
	"""
...
def strip_pincode(
	...
	"""
	Remove pincode from Address Line 2.
	"""
...
@frappe.whitelist()
def update_payment_entry_transaction_type(
	payment_entry,
	transaction_type,
):
	"""
	Save the edited Transaction Type back to
	the exact Payment Entry.custom_transaction_type.

	The field is a Data field, so any text value
	can be stored.
	"""

	if not payment_entry:
		frappe.throw(
			_("Payment Entry is required.")
		)
	...
	if not frappe.db.exists(
		...
		frappe.throw(
			_("Payment Entry {0} does not exist.").format(
				payment_entry
			)
		)
	...
	if not frappe.has_permission(
		...
		frappe.throw(
			_(
				"You do not have permission to update Payment Entry {0}."
			).format(
				payment_entry
			),
	...
	# Update ONLY custom_transaction_type.
	frappe.db.set_value(
		"Payment Entry",
		payment_entry,
		"custom_transaction_type",

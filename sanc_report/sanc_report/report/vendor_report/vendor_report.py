...
def get_columns():
	"""
	Updated Vendor Report columns.

	All four Blank columns have been removed completely.
	"""
...
def get_data(filters):
	...
	for row in raw_rows:
		...
		# Payment Entry.bank_account
		bene_bank_branch_name = row.get(
			"bene_bank_branch_name"
		)

		# Payment Entry.contact_email
		...
		# ------------------------------------------------------------
		# Notepad CSV Data
		# All 4 Blank columns removed from here also
		# ------------------------------------------------------------
...
def get_raw_rows(filters):
	"""
	Get submitted Payment Entry records.

	Data is taken ONLY where:
		party_type = Supplier

	Requested Payment Entry mappings:
	...
	for pe in payment_entries:
		...
		# Since data should be Supplier based,
		# address is fetched using the Supplier
		address = get_supplier_address(pe.party)
		...
		# Remove pincode from Address 2 completely.
		# Pincode will remain only in Bene Address 5.
		bene_address_2 = remove_pincode(
			bene_address_2,
			bincode,
		)
		...
		rows.append(
			{
				...
				# Kept blank because no new IFSC mapping
				# was requested.
				"ifsc_code": "",
...
def remove_pincode(address, pincode=None):
	"""
	Remove the supplied pincode from address text.

	Also removes any standalone Indian 6-digit pincode
	that may already be present in the address.
	"""
	...
	# First remove the actual Address.pincode
	if pincode:
	...
	# Remove any remaining standalone 6-digit pincode
	address = re.sub(
		r"\b\d{6}\b",
		"",
		address,
...
def get_transaction_type_code(
	custom_transaction_type,
):
	"""
	Map Payment Entry.custom_transaction_type
	to RBI adapter transaction code.
...
def get_supplier_address(supplier):
	"""
	Get Address linked specifically to the Supplier.
